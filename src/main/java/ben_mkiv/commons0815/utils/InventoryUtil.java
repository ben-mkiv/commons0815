...
public class InventoryUtil {
    ...
    public static ItemStack suckInventory(ItemStack filter,IInventory inv, int maxnum, EnumFacing access) {
        ...
        for(int i=0;i<inv.getSizeInventory() && maxnum>0;i+=1){
            ...
            if(!slot.isEmpty() && (filter==null || (filter!=null && filter.isItemEqual(slot)))){
                if(!(!(inv instanceof ISidedInventory) || ((inv instanceof ISidedInventory) && ((ISidedInventory)inv).canInsertItem(slots[i], slot, access))))
                    continue;
    ...
    public static int putInventory(ItemStack stack, IInventory inv, int maxnum, EnumFacing access, int[] slots){
        int maxcount = maxnum;
        for(int i : slots){
            if(!(!(inv instanceof ISidedInventory) || ((ISidedInventory)inv).canInsertItem(slots[i], stack, access)))
                continue;
    ...
    public static void addToPlayerInventory(ItemStack stack, EntityPlayer player) {
        if (stack != null) {
            ...
            if (stack.getCount() > 0) {
                player.dropItem(stack, false); //this was dropPlayerItemWithRandomChoice before
            }
        }
    }
